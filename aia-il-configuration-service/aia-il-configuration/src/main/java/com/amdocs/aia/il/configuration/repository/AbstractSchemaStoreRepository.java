...
public abstract class AbstractSchemaStoreRepository<M extends AbstractIntegrationLayerSchemaStoreModel> extends AiaProjectElementRepository<M> {
    ...
    protected AbstractSchemaStoreRepository(final AiaRepositoryOperations operations, final Class<M> mClass) {
        super(operations);
        this.mClass = mClass;
    }
    ...
    public M getByKey(final String projectKey, final String key) {
        ...
        properties.put(PROJECT_KEY, projectKey);
        properties.put("schemaStoreKey", key);
        properties.put(ELEMENT_TYPE, mClass.getSimpleName());
        properties.put(PRODUCTKEY_KEY, PRODUCT_KEY);
