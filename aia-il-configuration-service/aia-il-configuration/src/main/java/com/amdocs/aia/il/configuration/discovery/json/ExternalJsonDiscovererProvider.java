package com.amdocs.aia.il.configuration.discovery.json;

import com.amdocs.aia.il.common.model.external.ExternalSchemaType;
import com.amdocs.aia.il.configuration.discovery.AbstractDiscovererProvider;
import com.google.common.collect.Sets;
import org.springframework.stereotype.Component;

import java.util.Set;

@Component
public class ExternalJsonDiscovererProvider extends AbstractDiscovererProvider<ExternalJsonDiscoverer> {

    @Override
    public Set<ExternalSchemaType> getSupportedExternalSchemaTypes() {
        return Sets.immutableEnumSet(
                ExternalSchemaType.JSON_OVER_KAFKA,
                ExternalSchemaType.CATALOG1,
                ExternalSchemaType.DIGITAL1);
    }
}
