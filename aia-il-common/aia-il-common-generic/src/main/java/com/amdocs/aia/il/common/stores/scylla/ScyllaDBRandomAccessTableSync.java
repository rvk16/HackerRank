...
public class ScyllaDBRandomAccessTableSync implements RandomAccessTable, Serializable {
    private static final long serialVersionUID = 3768807787769530803L;

    private static final Logger LOGGER = LoggerFactory.getLogger(ScyllaDBRandomAccessTableSync.class);
    ...
    public ScyllaDBRandomAccessTableSync(ConfigurationRow configurationRow, JsonRepeatedMessageFormatter jsonRepeatedMessageFormatter) {
    ...
    public Collection<RepeatedMessage> queryData(String mainTable, Collection<KeyColumn> rowsToQuery, Long batchProcessingTimestamp) {
        ...
        try {
            rowsToQuery.forEach(rowKey -> {
                ...
                final ResultSet resultSet = session.execute(preparedStatement.bind(mainTable, strRowKey));
                ...
                if (!current.isEmpty()) {
                    ...
                    if (isUpdateTimeGreaterThanExistingTime(updateTime, existingUpdateTime, dayLightSaving)) {
                        ...
                        if (currentMessage != null) {
                            ...
                            updatedForeignKeys = deleteOldDataAndForeignKeys(session, scyllaQueryGeneratorHolder,
    ...
    private Map<Integer, KeyColumnDescriptor> deleteOldDataAndForeignKeys(final CqlSession session, ScyllaQueryGeneratorHolder holder, RepeatedMessage existingMessage, RepeatedMessage currentMessage, Double existingMessageTsVersion, Long dataBaseTimeStamp, String rowKey, final List<ResultSet> resultSets, double[] messageCountArr) {  //NOSONAR
        ...
        if (!isReferenceData()) {
            resultSets.add(session.execute(psQueryForInsertWithTTL.bind(this.tableName, rowKey, existingMessageTsVersion, existingMessageBytes, this.tableName, Long.parseLong(dataBaseTimeStamp + MICRO_POSTFIX))));
        }
        ...
        for (int j = 0; j < indexedFks.size(); j++) {
            ...
            if (!newValues.compareTo(oldValues)) {
                ...
                resultSets.add(session.execute(psQueryForInsertRelationWithTTL.bind(mainTableName,
                ...
                if (!isMTMRelation || isDoPropagation) {
                    ...
                    resultSets.add(session.execute(psQueryForInsertRelationWithTTL.bind(relTable,
    ...
    private void insertDataIntoDatabase(final CqlSession session, ScyllaQueryGeneratorHolder holder, RepeatedMessage currentMessage, RepeatedMessage existingMessage, String rowKey, Long dataBaseTimeStamp, Map<Integer, KeyColumnDescriptor> updatedForeignKeys, final List<ResultSet> resultSets, double[] messageCountArr) { //NOSONAR
        String currentMessageJson = serialize(currentMessage);
        ScyllaQueryGenerator qGen = (ScyllaQueryGenerator) holder.getQueryGenerator(configurationRow.getEntityStore().getSchemaStoreKey());
        PreparedStatement psQueryForInsert = qGen.getPsForInsert();

        resultSets.add(session.execute(psQueryForInsert.bind(this.tableName, rowKey,
                dataBaseTimeStamp.doubleValue(), currentMessageJson, this.tableName, Long.parseLong(dataBaseTimeStamp + MICRO_POSTFIX))));
        // main data counter
        messageCountArr[0]++;

        if (existingMessage == null) {
            handleInsertsForAllForeignKeys(session, qGen, currentMessage, resultSets, dataBaseTimeStamp, messageCountArr);
        } else {
            handleInsertsForUpdatedKeys(session, qGen, updatedForeignKeys, currentMessage, resultSets, dataBaseTimeStamp, messageCountArr);
        }
    }

    private void handleInsertsForAllForeignKeys(final CqlSession session, ScyllaQueryGenerator qGen, RepeatedMessage currentMessage,
                                                final List<ResultSet> resultSets, Long dataBaseTimeStamp,
        ...
        for (int j = 0; j < indexedFks.size(); j++) {
            ...
            resultSets.add(session.execute(psQueryForInsertRelation.bind(mainTableName,
    ...
    private void handleInsertsForUpdatedKeys(final CqlSession session, ScyllaQueryGenerator qGen, Map<Integer, KeyColumnDescriptor> updatedForeignKeys,
                                             RepeatedMessage currentMessage, final List<ResultSet> resultSets,
        ...
        for (Map.Entry<Integer, KeyColumnDescriptor> entry : updatedForeignKeys.entrySet()) {
            ...
            if (isCounterRelationUpsertNeeded(isMTMRelation, isDoPropagation)) {
                ...
                resultSets.add(session.execute(psQueryForInsertRelation.bind(relTable,
    ...
    public Map<KeyColumn, List<KeyColumn>> queryListChildData(String mainTable, Collection<KeyColumn> rowKeys, String contextName,
        ...
        final Map<KeyColumn, ResultSet> resultSets = new HashMap<>();
        ...
        try {
            rowKeys.forEach(rowKey -> {
                ...
                resultSets.put(rowKey, session.execute(ps1.bind(mainTable, strRowKey, contextName,
    ...
    public List<KeyColumn> queryListChildDataWithTTLAndWriteTimeChecked(String mainTable, Collection<KeyColumn> rowKeys, String contextName,
        ...
        try {
            rowKeys.forEach(rowKey -> {
                ...
                final ResultSet resultSet = session.execute(ps1.bind(mainTable, strRowKey, contextName,
                        relType, relationTable));
                resultSets.add(resultSet);
            });
            if (!resultSets.isEmpty()) {
                extractKeyColumnsFromResultSetWithTTLAndWriteTimeChecked(resultSets, matches, Long.parseLong(batchProcessingTimestamp + MICRO_POSTFIX));
                if(RealtimeDataPublisherCounterManager.INSTANCE!=null) {
                    RealtimeDataPublisherCounterManager.INSTANCE.getRecordsAccumulator().
                            addCounter(CounterType.REPLICATOR, CounterType.UPSERTREADCOMPLETED, configurationRow.getEntityStore().getSchemaStoreKey().replace(PUBLISHER_STORE, EMPTY),
                                    Long.parseLong(resultSets.size() + EMPTY));
    ...
    private static void extractKeyColumnsFromResultSet(final Map<KeyColumn, ResultSet> resultSets,
                                                       final Map<KeyColumn, List<KeyColumn>> matches, Long batchProcessingTimestamp) {
        for (final Map.Entry<KeyColumn, ResultSet> resultSet : resultSets.entrySet()) {
            List<KeyColumn> currentMatches = new ArrayList<>();
            processRows(resultSet.getValue(), currentMatches, batchProcessingTimestamp);
            matches.put(resultSet.getKey(), currentMatches);
        }
    }

    private static void processRows(final ResultSet rs, final List<KeyColumn> matches, Long batchProcessingTimestamp) {
    ...
    private static void extractKeyColumnsFromResultSetWithTTLAndWriteTimeChecked(final List<ResultSet> resultSets,
                                                                                 final List<KeyColumn> matches,
                                                                                 final long batchProcessingTimestamp) {
        for (final ResultSet resultSet : resultSets) {
            processRows(resultSet, matches, batchProcessingTimestamp);
        }
    }

    private static void processRows(final ResultSet rs, final List<KeyColumn> matches, final long batchProcessingTimestamp) {
        Iterator<Row> iterator = rs.iterator();
        while (iterator.hasNext()) {
            final Row row = iterator.next();
            ...
            if ((ttl == null || Long.parseLong(writeTime.toString()) >= batchProcessingTimestamp) && tsVersion <= batchProcessingTimestamp) {
                matches.add(new KeyColumn(ROWKEY_PATTERN.split(relationKey)));
            }
        }
    }

    private static void extractKeyColumnsFromResultSet(final List<ResultSet> resultSets,
                                                       final List<RelationKeyInfo> matches, final List<KeyColumn> sourceRowKeys) {
        int index = 0;
        for (final ResultSet resultSet : resultSets) {
            processRows(resultSet, matches, sourceRowKeys.get(index));
            ++index;
        }
    }

    private static void processRows(final ResultSet rs, final List<RelationKeyInfo> matches, final KeyColumn sourceRowKey) {
        Iterator<Row> iterator = rs.iterator();
        while (iterator.hasNext()) {
            final Row row = iterator.next();
            final String relationKey = row.getString(0);
    ...
    private void extractRepeatedMessagesFromResultSet(final List<ResultSet> resultSets, List<RepeatedMessage> matches, Long batchProcessingTimestamp) {
        for (final ResultSet resultSet : resultSets) {
            final Iterator<Row> iter = resultSet.iterator();
            double selectedVersion = 0L;
            Row selectedRow = null;
            while(iter.hasNext()) {
                final Row row = iter.next();
                double tsVersion = row.getDouble(1);
    ...
    private void extractRepeatedMessagesFromResultSetWithTTLAndWriteTimeChecked(final List<ResultSet> resultSets,
                                                                                List<RepeatedMessage> matches, Long batchProcessingTimestamp) {
        for (final ResultSet resultSet : resultSets) {
            final Iterator<Row> iter = resultSet.iterator();
            double selectedVersion = 0L;
            Row selectedRow = null;
    ...
    private List<List<Pair<RepeatedMessage, Double>>> queryList(List<String> keyList, Long batchProcessingTimestamp, double[] messageCountArr) {
        ...
        try {
            keyList.forEach(rowKey -> {
                ...
                resultSets.add(session.execute(psQuery.bind(configurationRow.getTableName(), rowKey)));
            });

            long futuresStartTime = System.currentTimeMillis();
            counter = getCounter(batchProcessingTimestamp, matchesList, resultSets, counter);
            if(RealtimeDataPublisherCounterManager.INSTANCE!=null) {
                RealtimeDataPublisherCounterManager.INSTANCE.getRecordsAccumulator().
                        addCounter(CounterType.REPLICATOR, CounterType.UPSERTREADCOMPLETED, configurationRow.getEntityStore().getSchemaStoreKey().replace(PUBLISHER_STORE, EMPTY),
                                Long.parseLong(resultSets.size() + EMPTY));
    ...
    private int getCounter(Long batchProcessingTimestamp, List<List<Pair<RepeatedMessage, Double>>> matchesList, List<ResultSet> resultSets, int counter) {
        for (final ResultSet resultSet : resultSets) {
            //Adding Null Check to facilitate reference store job since it does not use RealtimeDataPublisherCounterManager for metrices.
            List<Pair<RepeatedMessage, Double>> matches = new ArrayList<>();
            final Iterator<Row> iterator = resultSet.iterator();
            double selectedVersion = 0;
            Row selectedRow = null;
            while(iterator.hasNext()) {
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("Data retrieved from table {}", configurationRow.getTableName());
                }
                final Row row = iterator.next();
    ...
    public void updateBulkProcessedFlag(long batchTimestamp, List<BulkRelationInfo> bulkRelationInfos, double[] messageCountArr) {
        ...
        for (BulkRelationInfo bulkRelationInfo : bulkRelationInfos) {
            final ResultSet resultSet = session.execute(psQueryForInsertRelationWithTTL.bind(bulkRelationInfo.getMainTable(), buildDataBaseRowKey(bulkRelationInfo.getRelationKeyInfo().getSourceKeys().getIds()), bulkRelationInfo.getContextName(), bulkRelationInfo.getRelType(), bulkRelationInfo.getRelationTable(), buildDataBaseRowKey(bulkRelationInfo.getRelationKeyInfo().getRelationKey().getIds()), bulkRelationInfo.getRelationKeyInfo().getTsVersion(), bulkRelationInfo.getMainTable()));
            resultSets.add(resultSet);
        }
        RealtimeDataPublisherCounterManager.INSTANCE.getRecordsAccumulator().
                addCounter(CounterType.REPLICATOR, CounterType.UPSERTWRITECOMPLETED, configurationRow.getEntityStore().getSchemaStoreKey().replace(PUBLISHER_STORE, EMPTY),
        ...
        for (BulkRelationInfo bulkRelationInfo : bulkRelationInfos) {
            resultSets.add(session.execute(psInsertRelationWithBulkProcessed.bind(bulkRelationInfo.getMainTable(), buildDataBaseRowKey(bulkRelationInfo.getRelationKeyInfo().getSourceKeys().getIds()), bulkRelationInfo.getContextName(), bulkRelationInfo.getRelType(), bulkRelationInfo.getRelationTable(), buildDataBaseRowKey(bulkRelationInfo.getRelationKeyInfo().getRelationKey().getIds()), bulkRelationInfo.getRelationKeyInfo().getTsVersion(), bulkRelationInfo.getMainTable())));
        }
    }
    ...
    private static double getExistingTimeStamp(ScyllaQueryGenerator qGen, String mainTableName, String rowKeyFromExistingMessage, String ctx, String relType, String relTable, String relationKeyFromExistingMessage, double existingMessageTsVersion) { //NOSONAR
    ...
    private void insertDataIntoDatabaseWithTTL(final CqlSession session, ScyllaQueryGeneratorHolder holder, RepeatedMessage currentMessage, String rowKey, Long dataBaseTimeStamp, Double existingMessageTsVersion, final List<ResultSet> resultSets, double[] messageCountArr) { //NOSONAR
    ...
    public double[] deleteRowsUsingTTL(Map<Long, List<DeletedRowInfo>> deletedRowInfosPerBatch) {
        ...
        deletedRowInfosPerBatch.forEach((key, value) -> {
            for (DeletedRowInfo deletedRowInfo : value) {
                markRowUsingTTLAndWriteTime(session, holder, resultSets, deletedRowInfo, key, messagesCountArr);
            }
        });
        RealtimeDataPublisherCounterManager.INSTANCE.getRecordsAccumulator().
                addCounter(CounterType.REPLICATOR, CounterType.UPSERTWRITECOMPLETED, configurationRow.getEntityStore().getSchemaStoreKey().replace(PUBLISHER_STORE, EMPTY),
    ...
    private void markRowUsingTTLAndWriteTime(final CqlSession session, ScyllaQueryGeneratorHolder holder,
                                             final List<ResultSet> resultSets, DeletedRowInfo deletedRowInfo,
        ...
        resultSets.add(session.execute(psQueryForInsert.bind(this.tableName, deletedRowInfo.getRowKey(),
    ...
    public void registerFkIndex(List<KeyColumnDescriptor> indexedFks, List<String> contextNames, List<String> mainTableNames, List<String> relationTables, List<String> relationTypes, List<KeyColumnDescriptor> relationKey, List<Boolean> isMainRelation, List<Boolean> doPropagation, List<String> counterTables, List<Boolean> isMTM ){ //NOSONAR
        this.indexedFks.addAll(indexedFks);
        this.relationTypes.addAll(relationTypes);
        this.contextNames.addAll(contextNames);
        this.relationKey.addAll(relationKey);
        this.mainTableNames.addAll(mainTableNames);
        this.relationTables.addAll(relationTables);
        this.isMainRelation.addAll(isMainRelation);
        this.doPropagation.addAll(doPropagation);
        this.counterTables.addAll(counterTables);
        this.isMTM.addAll(isMTM);
    }
